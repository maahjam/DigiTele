<template>
  <div>
    <ReplyView
      class="w-1/2 pl-36"
      v-if="message.replyMessage !== null"
      :message="message.replyMessage"
      :contact="contact"
    />
    <div
      id="show-reply"
      class="bg-white cursor-pointer messageItem flex flex-row-revrese pt-2 pb-2 pl-24 pr-24"
      :class="
        selectedMessageId == message.id.toString() ? 'selected' : 'not-selected'
      "
      ...
      <div>
        <div class="circular-image">
          <img
            ...
            :src="message.isMine ? profile.photoURL : contact.photoURL"
            alt="Profile image"
          />
        </div>
      </div>
      ...
      <div class="w-full pl-4 pr-10">
        <div class="flex flex-row-revrese">
          ...
          <div class="text-right text-gray-400 w-1/2 clock">
            {{ getDateTimeString(message.dateTime) }}
          </div>
        </div>
